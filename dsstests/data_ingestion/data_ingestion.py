...
__author__      = 'A. N. Belikov'
__comment__     = 'Modifications to original code by J. C. Gonzalez'
...
#                self.conn.request(self.action, self.url, headers=self.sendheader)
        ...
        if not self.result:
            ...
            if self.nextsend:
                ...
                try:
                    TS=time.time()
                    if self.chksum: self.md5 = hashlib.md5()
                    data = self.f.read(self.write_buffer_size)
                    while data:
                        self.bcount += len(data)
                        if self.chksum: self.md5.update(data)
                        rcount += 1
                        while data:
...
#            self.sendheader['content-length'] = int(os.fstat(self.f.fileno())[6])
        ...
        if action in  ["STORED","DSSSTORED"] and self.storekey:
            ...
            #if self.md5sum:
            self.sendheader['MD5SUM'] = '%s' % (self.md5)
    ...
    def checksum(self, path):
        ...
        for i in ir:
            ...
            if d['path'] != '?': r.append((d['ip'], d['port'], d['path'], self.md5(d['path'], d['ip'], int(d['port']))))
        return r
