...
#    if table[uvars['operation']] not in ['getcaps', 'getid', 'getstats', 'ping', 'release', 'reload', 'testcache', 'teststore'] and not uvars['filename']: raise Exception, 'Need filename'
    if table_dss[uvars['operation']] not in ['ping','dsstestget','dsstestnetwork','dsstestconnection'] and not uvars['filename']: raise Exception, 'Need filename'
    if uvars['server'].find("://") >-1:
        ...
        if protocol=='https':
            ...
            uvars['secure']=False
        uvars['server']=server
    ds_connect = Data_IO(uvars['server'], debug=uvars['debug'], secure=uvars['secure'], certfile=uvars['certfile'],timeout=uvars['timeout'],looptime=uvars['looptime'],logfile=uvars['logfile'])
    errormes=''
    try:
        if uvars['filename']:
            fd = None
            ...
            result = getattr(ds_connect, table_dss[uvars['operation']])(username=uvars['username'], password=uvars['password'])
