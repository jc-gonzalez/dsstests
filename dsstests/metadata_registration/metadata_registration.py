...
class XmlMetadata(object):
    ...
    def __init__(self, path, filename, cfgfile):
        ...
        self.inputPath = path
        self.outputPath = XmlMetadata.cfg.get('testValues', 'outputPath')
        self.fieldnames = []
        self.result = {}
    ...
    def ingest(self, LOGFILE):
        ...
        if XmlMetadata.SDC == 'SOC':
            sdcUrl = '@easdps02.esac.esa.int:8001'
            # '@easdps-mis-' + Xml.ENVIRONMENT + '.esac.esa.int'
        elif XmlMetadata.SDC == 'SDC-NL':
            sdcUrl = '@eas-dps-mis.' + XmlMetadata.ENVIRONMENT + '.euclid.astro.rug.nl'
        else:
        ...
        client = xmlrpclib.ServerProxy('https://' + XmlMetadata.USERNAME + ':' +
                                       XmlMetadata.PASSWORD + sdcUrl)
        ...
        res = client.IngestObject(f_line, XmlMetadata.PROJECT, True)
        XmlMetadata.LOGGER.info(res)
    ...
    def retrieve(self, query, RETRIEVELOGFILE):
        ...
        # Define the server where to ingest data
        if XmlMetadata.SDC == 'SOC':
            sdcUrl = 'http://easdps-cus.' + XmlMetadata.ENVIRONMENT + '.esac.esa.int'
            # '@easdps-mis-' + Xml.ENVIRONMENT + '.esac.esa.int'
        elif XmlMetadata.SDC == 'SDC-NL':
            sdcUrl = 'http://eas-dps-cus.' + XmlMetadata.ENVIRONMENT + \
                     '.euclid.astro.rug.nl'
        else:
            raise ValueError('You entered a SDC that do not exist or is not '
                             'handled yet by this program please contact '
                             'magaly.alonzo@capgemini.com')
        url =  sdcUrl + '/EuclidXML?project=' + XmlMetadata.PROJECT + \
    ...
    def update(self, field, newValue):
        ...
        client = xmlrpclib.ServerProxy('https://' + XmlMetadata.USERNAME + ':' +
                                       XmlMetadata.PASSWORD +
                                       '@eas-dps-mis.' + XmlMetadata.ENVIRONMENT +
                                       '.euclid.astro.rug.nl')
        f_line = open(self.inputPath + self.filename).read()
        # Using client client function
        res = client.UpdateObject(f_line, XmlMetadata.PROJECT, True)
        XmlMetadata.LOGGER.info(res)
        return res, client
