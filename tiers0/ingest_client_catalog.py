...
import asyncore, errno, hashlib, httplib, os, fnmatch, pickle, signal, socket, stat, string, sys, time, urllib, urlparse, uuid, datetime
import xmlrpclib, codecs, ssl
...
Ingestserver = {
                 ...
                 'pip':'https://%s:%s@easdps-mis-pip.esac.esa.int',
                 }

Ingestserver = { 'current':'https://%s:%s@eas-dps-mis.euclid.astro.rug.nl', 
				'test':'https://%s:%s@eas-dps-mis.test.euclid.astro.rug.nl', 
				'dev':'https://%s:%s@eas-dps-mis.dev.euclid.astro.rug.nl', 
				'pip':'http://easdps-cus-pip.esac.esa.int', } 
...
class Data_IO:
    ...
    def _connect(self, host, port, secure):
        ...
        if secure:
            ...
            securetry = True
            if self.nocert:
                self.conn = httplib.HTTPSConnection(host, port, cert_file=self.certfile, timeout=self.timeout, context=ssl._create_unverified_context())
            else:
...
#                self.conn.request(self.action, self.url, headers=self.sendheader)
    ...
    def _respond(self):
        ...
        if self.status == 200:
            self.result = 0
            if self.action in ["GETLOCALSTATS", "GETGROUPSTATS", "GETTOTALSTATS", "LOCATE", "LOCATEFILE", "LOCATELOCAL", "LOCATEREMOTE", "MD5SUM", "PING", "SIZE", "STAT","DSSTESTGET","DSSTESTCONNECTION","DSSTESTNETWORK"]:
                ...
                pass
            elif self.action in ["STORE","DSSSTORE","DSSCACHEFILE"]:
                self.nextsend = True
                                ...
                                if self.result==0 and self.status== 200: 
                                    if 'jobstatus' in self.recvheader: 
                                        ...
                                        if self.jobstatus=='FINISHED':
                                            ...
                                            self.error_message='Unknown job status %s' % (jobstatus)
                                            putError()
                                            return False 
                                    else:
...
def checkfileexist(filename,ec_environment=None): 
    try:
        ingest_server=Cusserver[ec_environment]
    except:
        Message('Can not file CUS for your environment %s' % (ec_environment))
        return False
    url=ingest_server+'''/EuclidXML?class_name=DataFileName&DataFileName='%s'&PROJECT=DSS''' % (filename)
    response= urllib.urlopen(url)
    xml=response.read()
    if xml.find(filename)>-1: 
        return True 
    return False 


def store_datafile(path,localpath=None,ec_environment=None, SDC=None, username='', password='', debug=0,useoldfiles=False,nocert=False):
    if not path: 
    ...
    if checkfileexist(path,ec_environment=ec_environment): 
        if useoldfiles:
    ...
    if not os.path.isfile(check_path): 
        ...
        return False
    try:
        r = ds_connect.put(path=path, savepath=localpath, username=username, password=password)
...
def store_metadatafile(path,ec_environment=None, project='ALL', username=None, password=None, f_log=None):
    if not path: 
        Message('No file to ingest')
        return False 
    ...
    ingest_server=Ingestserver[ec_environment] % (username,password) 
    try: 
        sleep_time=1.0
        if os.path.exists(path):   
            if os.stat(path).st_size > 1000000L: 
                sleep_time=60.0
        ...
        res=client.IngestObjectWithCatalog(input_ascii_string,project,True)
        if f_log: 
            if not res['error']: 
                total_time=res['total_time']
                datamodel_time=res['datamodel_time']
                fileload_time=res['fileload_time']
                database_time=res['database_time']
                ...
                catalog_file=''
                catalog_id=''
                for t_cat in res['catalog']:
                    catalog_file_load=-1
                    fitsload=-1
                    fitsprocess=-1
                    dbingest=-1
                    dbcommit=-1
                    catalog_file=t_cat['catalog_file']
                    catalog_id=t_cat['catalog_id']
                    if not t_cat['error']:
                        catalog_file_load=t_cat['catalog_file_load']
                        i_cat=t_cat['catalog_ingest']
                        if i_cat: 
                            fitsload=i_cat['fitsload']
                            fitsprocess=i_cat['fitsprocess']
                            dbingest=i_cat['dbingest']
                            dbcommit=i_cat['dbcommit']
                    else: 
                        Message('Error ingesting catalog: %s' % t_cat['error'])
                    out_arr=[total_time,datamodel_time,fileload_time,database_time,ingest_time,catalog_file,catalog_id,catalog_file_load,fitsload,fitsprocess,dbingest,dbcommit]
                    out_str=','.join("%s" % k for k in out_arr)+'\n'
                    f_log.write(out_str)
                    cat_rows_cnt+=1
                if cat_rows_cnt==0: 
                    out_arr=[total_time,datamodel_time,fileload_time,database_time,ingest_time,catalog_file,catalog_id,catalog_file_load,fitsload,fitsprocess,dbingest,dbcommit]
                ...
                Message('Error ingesting XML : %s' % res['error'])


        if res['result']=='ok': 
            Message('XML metadata file ingested for %.1f seconds: %s' % (res['total_time'],path))
            return True
        elif res['error']:
            Message('Error ingesting XML file: %s - %s' % (path,res['message']+' '+res['error']))
            return False
        else:
            pass 
    except Exception, errmes:
...
def main():
    def usage():
        Message('''\nUsage: \n %s retrieve inputXMLfile|datafile  [--SDC=SDC_to_retriev_files_from] [--localdirectory=/path/to/store/file] [--environment=Euclid environment] [--noXML] [--project=EAS_project] [--nocert] [--logfile=/path/to/log/file]
 %s store  inputXMLfile|datafile|directory [--SDC=SDC_to_store_files] [--localdirectory=/path/to/local/files] [--environment=Euclid environment] [--noXML] [--username=username] [--password=password] [--createDDO] [--useoldfiles] [--project=EAS_project] [--nocert] [--logfile=/path/to/log/file]
    ...
    if len(sys.argv) < 3 or sys.argv[1] not in ['retrieve', 'store']:
        ...
        if logfile:
            if os.path.isfile(logfile):
                ...
                csv_head='total_time,datamodel_time,fileload_time,database_time,ingest_time,catalog_file,catalog_id,catalog_file_load,fitsload,fitsprocess,dbingest,dbcommit'
                f_log.write(csv_head+'\n')
        if os.path.isdir(path):
            ...
            for root, dirnames, inpfilenames in os.walk(path):
                ...
                for xmlfilename in fnmatch.filter(inpfilenames,'*.xml'):
                    ...
                    if not noXML and load_metadata:
                        load_metadata = store_metadatafile(os.path.join(root,xmlfilename),ec_environment, userproject, username, password, f_log)
                    else: 
            ...
            if noXML:
                ...
                if not noXML:
                    load_metadata = store_metadatafile(path,ec_environment,userproject, username, password, f_log)
                else: 
